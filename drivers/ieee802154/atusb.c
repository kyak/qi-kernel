/*
 * atusb.c - SPI host look-alike for ATSUB
 *
 * Written 2011 by Werner Almesberger <werner@almesberger.net>
 *
			...
			const struct usb_device_id *id)
{
	...
	dev = ieee802154_alloc_device(sizeof(struct atben_prv), &atusb_ops);
	if (!dev)
		return -ENOMEM;

	prv = dev->priv;
	prv->usb_dev = usb_dev;
	prv->udev = usb_get_dev(usb_dev);
	usb_set_intfdata(interface, prv);
